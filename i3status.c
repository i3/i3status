...
int main(int argc, char *argv[]) {
    ...
    cfg_opt_t mbox_opts[] = {
        CFG_STR("prefix", "✉: ", CFGF_NONE),
        CFG_STR("label", "default", CFGF_NONE),
        CFG_CUSTOM_ALIGN_OPT,
        CFG_CUSTOM_COLOR_OPTS,
        CFG_CUSTOM_MIN_WIDTH_OPT,
        CFG_END()};
    ...
    cfg_opt_t maildir_opts[] = {
        CFG_STR("prefix", "✉ ", CFGF_NONE),
        CFG_STR("format", "%dir:%count", CFGF_NONE),
        CFG_STR("separator", ", ", CFGF_NONE),
        CFG_STR("no_mail", "", CFGF_NONE),
        CFG_STR("exclude", "drafts spam trash", CFGF_NONE),
        CFG_CUSTOM_ALIGN_OPT,
        CFG_CUSTOM_COLOR_OPTS,
        CFG_CUSTOM_MIN_WIDTH_OPT,
        CFG_END()};
    ...
    while (1) {
        ...
        for (j = 0; j < cfg_size(cfg, "order"); j++) {
            ...
            CASE_SEC_TITLE("mbox") {
                SEC_OPEN_MAP("mbox");
                print_mbox_info(json_gen, buffer, title, cfg_getstr(sec, "prefix"), cfg_getstr(sec, "label"));
                SEC_CLOSE_MAP;
            }

            CASE_SEC_TITLE("maildir") {
                SEC_OPEN_MAP("maildir");
                print_maildir_info(json_gen, buffer, title, cfg_getstr(sec, "prefix"), cfg_getstr(sec, "format"), cfg_getstr(sec, "separator"), cfg_getstr(sec, "no_mail"), cfg_getstr(sec, "exclude"));
                SEC_CLOSE_MAP;
            }
