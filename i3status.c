...
int main(int argc, char *argv[]) {
    ...
    cfg_opt_t hostname_opts[] = {
        CFG_STR("format", "Hostname: %hostname", CFGF_NONE),
        CFG_CUSTOM_ALIGN_OPT,
        CFG_CUSTOM_COLOR_OPTS,
        CFG_CUSTOM_MIN_WIDTH_OPT,
        CFG_CUSTOM_SEPARATOR_OPT,
    ...
    cfg_opt_t opts[] = {
        ...
        CFG_SEC("hostname", hostname_opts, CFGF_NONE),
    ...
    while (1) {
        ...
        for (j = 0; j < cfg_size(cfg, "order"); j++) {
            ...
            CASE_SEC("hostname") {
                SEC_OPEN_MAP("hostname");
                print_hostname(json_gen, buffer, cfg_getstr(sec, "format"));
                SEC_CLOSE_MAP;
            }
        }
        if (output_format == O_I3BAR) {
